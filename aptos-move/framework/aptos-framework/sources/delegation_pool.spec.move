spec aptos_framework::delegation_pool {
    spec module {
        ...
        apply Null_Shareholder_Addr_Requirement to
        withdraw, add_stake, unlock, reactivate_stake, synchronize_delegation_pool;
        apply Global_Pending_Withdrawl_Owner to *;
        // TODO(Teng): long verification time
        apply Total_Coin_Inactive_LE_Inactive_Stake_Pool to
        withdraw, add_stake, unlock, reactivate_stake, synchronize_delegation_pool;
        apply Withdraw_Requirements  {
        ...
        // Global property 10 [TODO]:
        //

        // Global property 11 [ISSUE]:
        // total_coins_inactive <= inactive on stake: there could never be slashing on inactive stake
        // cannot be specified as global invariants, failed in stake::withdraw
        // moved to Total_Coin_Inactive_LE_Inactive_Stake_Pool as function-level invariants
    ...
    spec schema Global_Pending_Withdrawl_Owner {
    ...
    spec schema Total_Coin_Inactive_LE_Inactive_Stake_Pool {
        pool_address: address;
        // Global property 11
        // TODO(Teng): long verification time
    ...
    spec schema Withdraw_Requirements {
        pool: DelegationPool;
    ...
    spec schema Redeem_Inactive {
        pool: DelegationPool;
        ensures forall i in 0..pool.observed_lockup_cycle.index:
            table::spec_contains(pool.inactive_shares, ObservedLockupCycle{index: i}) ==>
                table::spec_get(pool.inactive_shares, ObservedLockupCycle{index: i}).total_coins
                    <= table::spec_get(old(pool).inactive_shares, ObservedLockupCycle{index: i}).total_coins;
    }
    ...
    spec add_stake(delegator: &signer, pool_address: address, amount: u64) {
        ...
        include Pool_Delegator_Address_Not_Eq {
        ...
        // Property 2 [OK: long verification time]: ensures active + pending_active == old(active) + old(pending_active) + amount
    ...
    spec unlock(delegator: &signer, pool_address: address, amount: u64) {
        ...
        requires delegator_address != NULL_SHAREHOLDER;
    ...
    spec withdraw_internal(pool: &mut DelegationPool, delegator_address: address, amount: u64) {
        ...
        let post stake_pool = spec_get_stake(pool);
        ensures !(withdrawal_exists &&
            (withdrawal_olc.index < old(pool).observed_lockup_cycle.index || spec_can_withdraw_pending_inactive(pool_address))) ==>
        ...
        // Property [OK]: if we do not withdraw pending_inactive, pending_inactive on the stakepool does not change after execution
