...
/// temporary workaround. TODO: more proper solution.
pub fn disable_speculative_logging() {
    DISABLE_SPECULATION.store(true, Ordering::SeqCst);
}

fn speculation_disabled() -> bool {
    DISABLE_SPECULATION.load(Ordering::SeqCst)
}
...
pub fn speculative_log(level: Level, context: &AdapterLogSchema, message: String) {
    ...
    if !context.speculation_supported() {
        // Speculation isn't supported in the current mode, log the entry directly.
        ...
        match &*BUFFERED_LOG_EVENTS.load() {
            ...
            None => {
                if speculation_disabled() {
                    // Speculation disabled globally, log directly. This is checked
                    // here to avoid overhead on the common path.
                    let log_event = VMLogEntry::new(level, context.clone(), message);
                    log_event.dispatch();
                } else {
                    alert!(
